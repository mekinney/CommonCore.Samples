    ...
    public class UserBusinessLogic : BusinessBase
    {
        ...
        public async Task<bool> Login(string userName, string password)
        {
            ...
            if (usr.Success && usr.Response != null)
            {
                ...
                {
                    success = true;
                    this.HttpService.Client.AddTokenHeader(AppSettings.AppUser.Token.Token);
                }
            }
            else
            {
        ...
        private async Task<bool> RefreshToken(string userName, string password)
        {
            ...
            if (success)
            {
                AppSettings.AppUser.Token = JsonConvert.DeserializeObject<AuthenticationToken>(httpResult.Response);
                this.HttpService.Client.AddTokenHeader(AppSettings.AppUser.Token.Token);
